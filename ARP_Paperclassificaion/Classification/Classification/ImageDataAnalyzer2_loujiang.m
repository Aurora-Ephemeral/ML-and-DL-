...
classdef ImageDataAnalyzer2_loujiang < handle
    properties
        imgRGB
        imgHSV
        img2HSV
...
%                 hold off
        ...
        function extractGrayFeatures(this)
            ...
            % Grauanteile
            whiteProp = sum(vHist(61:100)) / sum(this.imgBW, 'all');
            LightGrayProp = sum(vHist(40:60)) / sum(this.imgBW, 'all');
            MiddleGrayProp = sum(vHist(21:39)) / sum(this.imgBW, 'all');
            DarkGrayProp = sum(vHist(9:20)) / sum(this.imgBW, 'all');
            BlackProp = sum(vHist(2:8)) / sum(this.imgBW, 'all'); 
